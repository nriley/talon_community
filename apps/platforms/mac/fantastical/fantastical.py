			...
			if split.AXSplitters[0].AXValue == 0:
				...
				for attempt in range(10):
					if split.AXSplitters[0].AXValue > 0:
						break
					actions.sleep("50ms")
			parent = split.children.find_one(AXRole="AXGroup", AXDescription="sidebar",
											 max_depth=0)
		except:
			parent = window

		(parent.children.find_one(AXRole='AXPopUpButton', AXDescription='calendar set',
			max_depth=0)).perform('AXPress')
...
class Actions:
	...
	def fantastical_select_calendar_set(text: str): """Select a calendar set in Fantastical"""
